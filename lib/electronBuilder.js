...
function mainJsSource( { appId, appName, websiteUrl, backgroundColor, preloaderEnabled, minDurationMs, offlineGameEnabled } ) {
	return `const { app, BrowserWindow, session } = require('electron');
const path = require('path');
const dns = require('dns');

const PRELOADER_ENABLED = ${ JSON.stringify( !! preloaderEnabled ) };
const MIN_SPLASH_MS = ${ JSON.stringify( minDurationMs ) };
const OFFLINE_ENABLED = ${ JSON.stringify( !! offlineGameEnabled ) };
const REMOTE_URL = ${ JSON.stringify( websiteUrl ) };
const OFFLINE_RETRY_MS = 4000;

// Every generated app previously shared the literal package.json name
// "wta-generated-desktop-app", which Electron uses to pick the userData
// folder (%APPDATA%/<name> on Windows) — so every different site's app on
// the same machine shared one cookie/session store instead of each having
// its own. Give each app (by its unique appId) an isolated, stable one, and
// do it before app.whenReady() so it applies from the very first launch.
app.setName(${ JSON.stringify( appName ) });
app.setPath('userData', path.join(app.getPath('appData'), ${ JSON.stringify( appId ) }));

// Electron blocks window.open()/target=_blank popups by default — this
// breaks any "Login with Google/Facebook/..." button that opens the auth
// flow in a popup. Allow them through as normal windows.
...
// Swaps the window to the bundled offline game (no network needed, it's a
// local file) and keeps retrying the real site in the background until one
// of those retries actually succeeds.
function goOffline() {
	if (! OFFLINE_ENABLED || isOffline || ! main) return;
	isOffline = true;
	main.loadFile(path.join(__dirname, 'offline.html'));
	if (retryTimer) clearInterval(retryTimer);
	retryTimer = setInterval(() => {
		if (main && ! main.isDestroyed()) main.loadURL(REMOTE_URL);
	}, OFFLINE_RETRY_MS);
}
...
function createMain() {
	...
	main.webContents.on('did-fail-load', (event, errorCode, errorDescription, validatedURL, isMainFrame) => {
		if (! isMainFrame) return;
		if (validatedURL && validatedURL.startsWith('file://')) return;
		goOffline();
	});

	// Check connectivity before the very first load too — otherwise a
	// machine that's already offline when the app launches has to wait on
	// did-fail-load's own (slower, OS-dependent) timeout before the game
	// appears, instead of showing it immediately like Android does.
	if (OFFLINE_ENABLED) {
		const remoteHost = (() => { try { return new URL(REMOTE_URL).hostname; } catch (e) { return null; } })();
		if (remoteHost) {
			dns.lookup(remoteHost, (err) => {
				if (err) goOffline();
				else main.loadURL(REMOTE_URL);
			});
		} else {
			main.loadURL(REMOTE_URL);
		}
	} else {
		main.loadURL(REMOTE_URL);
	}
...
app.whenReady().then(() => {
	// Make logins survive closing/reopening the app. A cookie with no
	// Max-Age/Expires is a "session cookie" — Chromium (what Electron uses)
	// deletes those the moment the app fully quits, same as a desktop
	// browser closing. Most login systems set exactly this kind of cookie
	// since they assume "browser stays open" web behavior, not "app closes
	// completely between uses" native-app behavior. Rewrite any such cookie
	// to persist for 30 days instead — the site's own backend still decides
	// whether the *session* itself is still valid, this only controls how
	// long the browser keeps the cookie around locally.
	session.defaultSession.webRequest.onHeadersReceived((details, callback) => {
		const headers = details.responseHeaders || {};
		const setCookieKey = Object.keys(headers).find((k) => 'set-cookie' === k.toLowerCase());
		if (setCookieKey) {
			headers[setCookieKey] = headers[setCookieKey].map((cookie) =>
				/max-age=|expires=/i.test(cookie) ? cookie : cookie + '; Max-Age=2592000'
			);
		}
...
async function buildDesktop( job ) {
	...
	await fsp.writeFile(
		...
		mainJsSource( { appId, appName, websiteUrl, backgroundColor, preloaderEnabled, minDurationMs, offlineGameEnabled } )
	);

	// 1b. Offline game — shown automatically when the connection drops (see
	// main.js's did-fail-load handler above); bundled as a local file so it
	// needs no network itself.
	if ( offlineGameEnabled ) {
		const { getOfflineGameHtml } = require( './offlineGame' );
		await fsp.writeFile( path.join( workDir, 'offline.html' ), getOfflineGameHtml() );
	}
	...
	if ( fs.existsSync( path.join( workDir, 'offline.html' ) ) ) {
		sourceFiles.push( 'offline.html' );
	}
	...
	// 4. A real native installer for whichever OS this worker itself runs on —
	// electron-builder can only produce Mac installers on a Mac, Linux ones
	// need Linux (or at least won't reliably cross-build from Windows), so we
	// just build for process.platform and leave the others as source-only.
	const NATIVE_TARGET = {
		win32: { flag: '--win', match: ( f ) => f.endsWith( '.exe' ), suffix: 'desktop-win.exe', label: 'Windows (.exe)' },
		linux: { flag: '--linux', match: ( f ) => f.endsWith( '.AppImage' ) || f.endsWith( '.deb' ), suffix: null, label: 'Linux (AppImage/.deb)' },
		darwin: { flag: '--mac', match: ( f ) => f.endsWith( '.dmg' ), suffix: 'desktop-mac.dmg', label: 'macOS (.dmg)' },
	}[ process.platform ];

	let desktopWindowsUrl = null;
	let desktopLinuxUrl = null;
	let desktopMacUrl = null;

	if ( NATIVE_TARGET ) {
		await run( 'npm install --no-audit --no-fund', workDir, jobId );
		await run( `npx electron-builder ${ NATIVE_TARGET.flag } --publish=never`, workDir, jobId );

		const distDir = path.join( workDir, 'dist' );
		const built = fs.existsSync( distDir )
			? fs.readdirSync( distDir ).filter( NATIVE_TARGET.match )
			: [];

		if ( ! built.length ) {
			throw new Error( `electron-builder reported success but no matching artifact was found in dist/ for ${ process.platform }` );
		}

		for ( const file of built ) {
			const destName = `${ jobId }-${ NATIVE_TARGET.suffix || file }`;
			await fsp.copyFile( path.join( distDir, file ), path.join( DOWNLOADS_DIR, destName ) );
			const url = `${ baseUrl }/downloads/${ destName }`;
			log( jobId, `${ NATIVE_TARGET.label } artifact ready: ${ url }` );

			if ( 'win32' === process.platform ) desktopWindowsUrl = url;
			if ( 'linux' === process.platform && ! desktopLinuxUrl ) desktopLinuxUrl = url;
			if ( 'darwin' === process.platform ) desktopMacUrl = url;
		}
	}
	...
	return {
		...
		desktopMacUrl,
		desktopSourceUrl,
		desktopNote: NATIVE_TARGET
			? `Built natively for ${ NATIVE_TARGET.label } on this worker. Other desktop OSes only get the source project — unzip it on that OS and run \`npm install && npm run dist\`.`
			: 'This worker\'s OS isn\'t recognized for a native build — only the source project is available. Unzip it on Windows/macOS/Linux and run `npm install && npm run dist`.',
	};
}
