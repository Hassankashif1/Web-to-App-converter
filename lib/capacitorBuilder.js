...
async function buildAndroid( job ) {
	...
	// MainActivity is always patched — the cookie fix (login/session
	// persisting across app restarts) applies regardless of the offline-game
	// toggle; the offline-game network-detection code is added on top only
	// when that's enabled.
	{
		...
		if ( fs.existsSync( mainActivityPath ) ) {
			await fsp.writeFile( mainActivityPath, androidMainActivitySource( { packageName: appId, websiteUrl, offlineGameEnabled } ) );
			log( jobId, 'MainActivity.java patched: login/session cookies now persist across app restarts' + ( offlineGameEnabled ? ', and offline.html shows automatically when connectivity drops.' : '.' ) );
		} else {
			log( jobId, `MainActivity.java patch skipped: expected it at ${ mainActivityPath } but it wasn't there.` );
		}
